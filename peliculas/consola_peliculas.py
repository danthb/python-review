...
def ejecutar_consultar_duracion_promedio_peliculas(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict) -> None:
    ...
    Parametros:
        ...
        p5 (dict): Diccionario que contiene la informacion de la pelicula 5.
    """
    tiempo = int(mod.duracion_promedio_peliculas(p1, p2, p3, p4, p5))
    hora = str(tiempo//60)
    minutos = str(tiempo % 60)
    print('La duracion promedio de las peliculas es de: ' +
          hora + ':' + minutos + ' HHMM.')
...
def ejecutar_encontrar_estrenos(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict) -> None:
    ...
    Parametros:
        ...
        p5 (dict): Diccionario que contiene la informacion de la pelicula 5.
    """
    anio = int(input("Ingrese año de referencia para consultar estrenos: "))

    print(str(mod.encontrar_estrenos(p1, p2,
          p3, p4, p5, anio)))
...
def ejecutar_cuantas_peliculas_18_mas(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict) -> None:
    ...
    Parametros:
        ...
        p5 (dict): Diccionario que contiene la informacion de la pelicula 5.
    """
    print('Existen ' + str(mod.cuantas_peliculas_18_mas(p1, p2,
          p3, p4, p5)) + ' peliculas con clasificacion 18+')
...
def ejecutar_reagendar_pelicula(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict) -> None:
    ...
    if pelicula is None:
        ...
        print("Pelicula encontrada: " + pelicula["nombre"])
        nuevo_dia = input("Ingrese nuevo dia de estreno (DDMM): ")
        nueva_hora = int(input(
            "Ingrese nueva hora de estreno (HHMM) por favor en un formato de 24 horas:  "))

        while 1:
            deseaControl = input("Desea controlar la pelicula? (S/N): ")
            if deseaControl == "S":
                control = True
                break
            elif deseaControl == "N":
...
def ejecutar_decidir_invitar(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict) -> None:
    ...
    if pelicula is None:
        ...
        if edad_invitado >= 18:
            ...
            while 1:
                autorizacion = input(
                    "Tiene la autorización de los padres? (S/N): ")
                if autorizacion == "S":
                    autorizacion_padres = True
                    break
                elif autorizacion == "N":
        ...
        if mod.decidir_invitar(pelicula, edad_invitado, autorizacion_padres):
            ...
            print("No se pudo invitar al invitado")
